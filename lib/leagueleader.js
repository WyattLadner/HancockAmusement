...
function parseTables(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isRule(line)) {
      ...
      if (headerLine && isRow(headerLine) && isRule(lines[i + 2] || "")) {
        ...
        pendingTitle = [];
        // Resume ON the closing rule (i = j - 1, since the loop will i++ to j), so a
        // rule that is shared between two stacked page-blocks is also treated as the
        // NEXT block's opening rule — otherwise every block after a shared rule is lost.
        i = j - 1;
        continue;
      }
    }
  ...
  return mergeSplitTables(tables);
}

// A long report table is split across page-blocks that repeat the same header. Merge
// consecutive tables with identical headers back into one, keeping the first title.
function mergeSplitTables(tables) {
  const sameHeaders = (a, b) => a.length === b.length && a.every((h, i) => h === b[i]);
  const out = [];
  for (const t of tables) {
    const prev = out[out.length - 1];
    if (prev && sameHeaders(prev.headers, t.headers)) {
      prev.rows = prev.rows.concat(t.rows);
    } else {
      out.push({ title: t.title, headers: t.headers, rows: t.rows.slice() });
    }
  }
  return out;
}
