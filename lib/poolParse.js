...
export function titleCase(s) {
  // Capitalize the first letter of each space-separated word only — NOT the letter
  // after an apostrophe (so "bee's" stays "Bee's", not "Bee'S").
  return String(s)
    .trim()
    .replace(/\s+/g, " ")
    .toLowerCase()
    .replace(/(^|\s)([a-z0-9])/g, (_, pre, c) => pre + c.toUpperCase());
}
